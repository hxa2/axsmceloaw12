...
export interface MatrixStageProps {
  ...
  // Highlight dummy row/column (index 0-based)
  dummyRowIndex?: number | null
  dummyColIndex?: number | null
  ...
  return (
    <div id="matrix-stage-container" className="result-matrix-container" style={{ position: 'relative' }}>
      ...
      <div className="result-matrix-row">
        ...
        {dests.map((name, j) => {
          const isDummyCol = dummyColIndex !== null && dummyColIndex === j
          return (
            <div key={`col-${j}`} className="result-matrix-cell header" style={{
              ...
              {isDummyCol && (
                <span style={{
                  ...
                  background: 'var(--warning-bg, rgba(255,165,0,0.15))',
      ...
      {/* ── Main Grid ── */}
      {sources.map((name, i) => {
        const isDummyRow = dummyRowIndex !== null && dummyRowIndex === i
        return (
          <div key={`row-${i}`} className="result-matrix-row">
            
            {/* Row Header (Sources & u_i) */}
            <div className="result-matrix-cell header" style={{
              flexDirection: 'column', gap: 2,
              ...(isDummyRow ? {
                background: 'var(--bg-inset)',
                color: 'var(--text-tertiary)',
            ...
            {dests.map((_, j) => {
              ...
              const isDummyCell = (isDummyRow) || (dummyColIndex !== null && dummyColIndex === j)
              
              return (
                <div key={`cell-${i}-${j}`} className={getCellClasses(i, j)} style={{
                  ...(isDummyCell && !getCellClasses(i, j).includes('cell-entering') && !getCellClasses(i, j).includes('cell-leaving') ? {
                    background: 'var(--bg-inset)',
                    opacity: 0.7,
                    ...(isDummyRow ? { borderTop: '2px dashed var(--border-subtle)' } : {}),
                    ...(dummyColIndex !== null && dummyColIndex === j ? { borderLeft: '2px dashed var(--border-subtle)' } : {}),
                  } : {})
                }}>
            ...
            <div className="result-matrix-cell header" style={{
              ...
              ...(isDummyRow ? { fontStyle: 'italic', borderTop: '2px dashed var(--border-subtle)' } : {})
            }}>
              {editable ? (
                <input
                  id={`cell-${i}-supply`}
